...
def test_system():
    ...
    try:
        ...
        req = urllib.request.Request(
            'http://localhost:5000/register_user',
            data=json.dumps(doctor_data).encode(),
            headers={'Content-Type': 'application/json'}
        )
        response = urllib.request.urlopen(req, context=ssl_context)
        result = json.loads(response.read().decode())
        if result['success']:
            print(f"   [SUCCESS] 医生用户注册成功! 用户ID: {result['user_id']}")
        else:
            print(f"   [WARNING] 医生用户注册返回错误: {result.get('error', 'Unknown error')}")
        
        # 发送消息
        message_data = {
            'sender': 'test_patient_final',
            'receiver': 'test_doctor',
            'message': '这是一条测试消息'
        }
        req = urllib.request.Request(
